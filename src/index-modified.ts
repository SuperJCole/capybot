// (modified top portion only; the rest of this file can stay as you supplied)
import fs from "fs";
...
// DATA SOURCES
import { CoinbaseBTCtoUSDC } from "./data_sources/coinbase/CoinbaseBTCtoUSDC";

// POOLS
import { CetusPool } from "./dexs/cetus/cetus";
import { TurbosPool } from "./dexs/turbos/turbos";
import { SuiswapPool } from "./dexs/suiswap/suiswap"; // adjust path if different
...
// ───────────────────────────────────────────────────────────────
// WALLET / CONFIG (moved to central module)
// ───────────────────────────────────────────────────────────────

import { keypair } from "./config"; // <-- use the shared config module
export { keypair };
...
// ───────────────────────────────────────────────────────────────
// (the rest of index.ts remains the same as your version)
// ───────────────────────────────────────────────────────────────
...
// ───────────────────────────────────────────────────────────────
// POOLS — SAME STRUCTURE AS YOUR WORKING FILE
// ───────────────────────────────────────────────────────────────
...
// ★ NEW — SUISWAP POOL WIRED IN EXACTLY LIKE YOUR STYLE
const suiswapSUItoUSDC = new SuiswapPool(
...
capybot.addPool(suiswapSUItoUSDC);   // ★ ADDED
capybot.addPool(cetusWBTCtoUSDC);

// ───────────────────────────────────────────────────────────────
// REGISTER COINBASE AS THE PRICE SOURCE
// ───────────────────────────────────────────────────────────────

capybot.addDataSource(new CoinbaseBTCtoUSDC());
...
capybot.addStrategy(
  new RideTheTrend(
    ...
    5,
    10,
    [defaultAmount[cetusUSDCtoSUI.coinTypeA], defaultAmount[cetusUSDCtoSUI.coinTypeB]],
...
capybot.addStrategy(
  new RideTheTrend(
    ...
    5,
    10,
    [defaultAmount[cetusCETUStoSUI.coinTypeA], defaultAmount[cetusCETUStoSUI.coinTypeB]],
    RIDE_THE_TREND_LIMIT,
...
capybot.addStrategy(
  new RideTheTrend(
    ...
    5,
    10,
    [defaultAmount[cetusUSDCtoCETUS.coinTypeA], defaultAmount[cetusUSDCtoCETUS.coinTypeB]],
    RIDE_THE_TREND_LIMIT,
...
capybot.addStrategy(
  new Arbitrage(
    [
      { pool: turbosSUItoUSDC.uri, a2b: true },
      { pool: cetusUSDCtoSUI.uri, a2b: true },
    ],
    defaultAmount[coins.SUI],
    ARBITRAGE_RELATIVE_LIMIT,
...
capybot.addStrategy(
  new MarketDifference(
    ...
    "Market diff: (W)BTC/USDC, Coinbase vs CETUS"
  )
);

// External Trend Riding
capybot.addStrategy(
  new RideTheExternalTrend(
    ...
    RIDE_THE_TREND_LIMIT,
    1.0001,
    "Ride external trend: (W)BTC/USDC, Coinbase vs CETUS"
  )
);
